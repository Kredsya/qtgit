...
# 상단바의 help의 about클릭 시 Made by Antonin Desfontaines.를 출력하는 창을 띄우기 위한 클래스
...
class FileSystemModelWithGitStatus(QFileSystemModel):
    def __init__(self, parent=None):
        super().__init__(parent)
        # A dictionary to store git statuses, where the key is the filePath(QModelIndex) of file path
        self.git_statuses = {}

    def columnCount(self, parent=None):
        return super().columnCount() + 1  # Increase the column count by 1

    # QFileSystemModel의data함수를 오버라이딩#QFileSystemModel의4번째 열의 데이터를 git status로 설정
    ...
    # QFileSystemModel의headerData함수를 오버라이딩 #QFileSystemModel의4번째 열의 헤더를"Git Status"로 설정
    def headerData(self, section, orientation, role=Qt.DisplayRole):
        if section == 4 and role == Qt.DisplayRole:
            return "Git Status"
        return super().headerData(section, orientation, role)

    # FilesystemModelWithGitStatus의 git_statuses필드에 git status를 저장한 후 모델을 업데이트하는 함수- self.data이용
...
class App(QMainWindow):  # main application window를 위한 클래스
    def __init__(self, initialDir):
        ...
        # Variables
        # pathlib모듈의 Path객체를 받음. main함수에서 App객체를 생성할 때에는 home디렉토리(사용자 폴더)를 인자로 넘겨줌
    ...
    def initUI(self):  # UI를 초기화하는 함수
        ...
        # QDir.Dirs : 디렉토리만 표시하도록 필터링 # QDir.NoDotAndDotDot : "."과 ".."을 표시하지 않도록 필터링 # setFilter : 모델의 필터를 설정하는 함수
        dirModel.setFilter(QDir.Dirs | QDir.NoDotAndDotDot)
        # QDir.rootPath() : 루트 디렉토리의 경로를 반환하는 함수 # setRootPath : 모델의 루트 경로를 설정하는 함수
        dirModel.setRootPath(QDir.rootPath())

        # Side explorer
        # QTreeView : QAbstractItemView를 상속받은 클래스. 트리 형태로 데이터를 표시하는 위젯
        ...
        # QTreeView의 아이템을 클릭했을 때 navigate함수를 호출
        ...
        # QFileSystemModel : 파일 시스템의 디렉토리 구조를 표현하는 모델
        self.mainModel = FileSystemModelWithGitStatus()
        self.mainModel.setFilter(QDir.NoDotAndDotDot |
                                 QDir.AllDirs | QDir.Files | QDir.Hidden)
        # QDir.rootPath() : 루트 디렉토리의 경로를 반환하는 함수
        self.mainModel.setRootPath(QDir.rootPath())
        # QFileSystemModel의 읽기 전용 속성을 False로 설정
        self.mainModel.setReadOnly(False)
        # QFileSystemModel의 디렉토리가 로드될 때 updateStatus함수를 호출
        ...
        # QSplitter : 위젯을 분할하여 배치하는 클래스
        self.explorerSplitter = QSplitter(Qt.Horizontal)
        self.explorerSplitter.addWidget(
            self.sideExplorer)  # QSplitter에 QTreeView를 추가
        self.explorerSplitter.addWidget(
            self.mainExplorer)  # QSplitter에 QListView를 추가

        self.explorerSplitter.setStretchFactor(
            1, 2)  # QSplitter의 1번째 위젯의 크기를 2배로 설정
        self.explorerSplitter.setSizes([400, 800])  # QSplitter의 크기를 설정
        # QHBoxLayout에 QSplitter를 추가
        ...
        # QMainWindow의 디렉토리를 homePath로 설정
    ...
    def GitAdd(self):
        # QFileSystemModel의 현재 디렉토리의 경로를 반환하는 함수
        ...
        if os.path.isdir(path + '/.git'):
            ...
            QMessageBox.warning(
                self, "Warning", "git init을 먼저 하세요.", QMessageBox.Ok)
    ...
    def createActionBar(self):  # 상단바의 action바를 위한 메소드 - 상위 디렉토리로 가거나 현제 디렉토리를 표시하거나 검색하는 기능
        ...
        # 이전 디렉토리로 가는 버튼이나 이것과 connect된 메소드가 없어서 작동 X
        ...
        # 다음 디렉토리로 가는 버튼이나 이것과 connect된 메소드가 없어서 작동 X
        ...
        self._navigateUpButton.clicked.connect(
            self.navigateUp)  # 클릭 시 navigateUp메소드 실행

        # 상단바의 action바에 들어갈 위젯들을 위한 splitter
    ...
    # 상단바의 Edit의 selectAll 클릭시, QListView클래스의 selectAll함수 이용.
    def selectAll(self, event):
        self.mainExplorer.selectAll()

    # 상단바의 Edit의 unselectAll 클릭시, QListView클래스의 selectionModel함수 이용.
    def unselectAll(self):
        self.mainExplorer.selectionModel().clearSelection()

    def navigate(self, index):  # QListView클래스의 setRootIndex함수 이용.
        self.currentDir = self.mainModel.fileInfo(
            index).absoluteFilePath()  # 현재 디렉토리를 설정
        self.mainExplorer.setRootIndex(self.mainModel.setRootPath(
            self.currentDir))  # QListView의 디렉토리를 설정
        # QMainWindow의 타이틀을 현재 디렉토리로 설정
        self.setWindowTitle(os.path.basename(self.currentDir))
        # 현재 디렉토리를 표시하는 위젯에 현재 디렉토리를 설정
        self.addressBar.setText(self.currentDir)

    def navigateUp(self, event):  # 상위 디렉토리로 가는 메소드
        self.currentDir = os.path.dirname(
            self.currentDir)  # 현재 디렉토리의 상위 디렉토리를 설정
        self.navigate(self.mainModel.setRootPath(
            self.currentDir))  # QListView의 디렉토리를 설정
    ...
    def deleteFiles(self, event):  # 상단바의 Edit의 delete 클릭시, 선택된 파일들을 삭제하는 메소드
        ...
        if reply == QMessageBox.Yes:  # Yes버튼을 누르면
            selectedIndexes = self.mainExplorer.selectionModel(
            ).selectedIndexes()  # 선택된 파일들의 인덱스를 가져옴
    ...
    # 상단바의 View의 List, Icons, Details를 클릭시, QListView, QTableView의 모드를 변경하는 메소드
    def changeView(self, view):
        if (view == "List"):  # List를 클릭하면
            ...
            # QListView의 모드를 ListMode로 변경
            self.mainExplorer.setViewMode(QListView.ListMode)
            self.mainExplorer.setIconSize(
                QSize(64, 64))  # QListView의 아이콘 크기를 변경
        elif (view == "Icons"):  # Icons를 클릭하면
            if type(self.mainExplorer) is not QListView:  # QListView가 아니면
                self.mainExplorer = QListView()  # QListView로 변경

            # QListView의 모드를 IconMode로 변경
            self.mainExplorer.setViewMode(QListView.IconMode)
            self.mainExplorer.setWordWrap(True)  # QListView의 아이콘을 자동으로 줄바꿈
            self.mainExplorer.setIconSize(
                QSize(64, 80))  # QListView의 아이콘 크기를 변경
            self.mainExplorer.setUniformItemSizes(
                True)  # QListView의 아이콘 크기를 균일하게 변경
        elif (view == "Details"):  # Details를 클릭하면
            self.mainExplorer = QTableView()  # QTableView로 변경
            self.mainExplorer.setSelectionBehavior(
                QAbstractItemView.SelectRows)  # QTableView의 선택 모드를 행 단위로 변경
            self.mainExplorer.setSelectionMode(
                QAbstractItemView.ExtendedSelection)  # QTableView의 선택 모드를 다중 선택으로 변경

            self.mainExplorer.verticalHeader().hide()  # QTableView의 세로 헤더를 숨김
            self.mainExplorer.setShowGrid(False)  # QTableView의 그리드를 숨김
            self.mainExplorer.horizontalHeader().setSectionsMovable(
                True)  # QTableView의 헤더를 이동 가능하게 변경
            self.mainExplorer.horizontalHeader().setHighlightSections(
                False)  # QTableView의 헤더를 선택시 하이라이트를 표시하지 않게 변경
            self.mainExplorer.setFrameStyle(
                QFrame.NoFrame)  # QTableView의 테두리를 숨김
            self.mainExplorer.setSortingEnabled(True)  # QTableView의 정렬을 활성화
            self.mainExplorer.setEditTriggers(
                QAbstractItemView.EditKeyPressed)  # QTableView의 편집을 키보드로 활성화

        # Common
        # QListView, QTableView의 더블 클릭 이벤트를 처리하는 함수를 연결
        self.mainExplorer.doubleClicked.connect(self.onDoubleClick)
        # QListView, QTableView의 컨텍스트 메뉴를 커스텀으로 변경
        self.mainExplorer.setContextMenuPolicy(Qt.CustomContextMenu)
        self.mainExplorer.customContextMenuRequested.connect(
            self.contextItemMenu)  # QListView, QTableView의 컨텍스트 메뉴 이벤트를 처리하는 함수를 연결
        # QListView, QTableView의 모델을 설정
        self.mainExplorer.setModel(self.mainModel)

        if hasattr(self, "explorerSplitter"):  # explorerSplitter가 존재하면
            # explorerSplitter의 1번째 위젯을 QListView, QTableView로 변경
            self.explorerSplitter.replaceWidget(1, self.mainExplorer)
            self.mainExplorer.setRootIndex(self.mainModel.setRootPath(
                self.currentDir))  # QListView, QTableView의 루트 경로를 설정

    def updateStatus(self, path):  # 하단바의 상태를 업데이트하는 메소드
        # 현재 폴더의 파일 개수를 가져옴 #QListView, QTableView의 파일 개수를 가져옴
        status = str(self.mainModel.rowCount(
            self.mainModel.index(path))) + " elements"
        # 선택된 파일 개수를 가져옴 #QListView, QTableView의 선택된 파일 개수를 가져옴
        selectedCount = len(
            self.mainExplorer.selectionModel().selectedIndexes())
        if selectedCount > 0:  # 선택된 파일이 있으면
            status += " | " + str(selectedCount) + \
                " elements selected"  # 선택된 파일 개수를 상태에 추가
    ...
    def onDoubleClick(self, event):  # 더블클릭 이벤트 처리
        ...
        # QFileInfo 객체로부터 절대 경로를 얻고 문자열로 변환
        itemPath_str = str(itemPath.absoluteFilePath())
        if isdir(itemPath):  # 더블클릭한 파일이 폴더일 경우
            self.navigate(event)  # navigate함수 호출 #폴더를 열어줌
            # os.path.isdir() : 디렉토리가 존재하는지 확인하는 함수
            if os.path.isdir(itemPath_str + "/.git"):
    ...
    # 선택된 파일을 mainExplorer.selectionModel().selectedIndexes()로 받아와서 이름을 바꿔줌
    def renameFile(self, event):
        selectedIndexes = self.mainExplorer.selectionModel(
        ).selectedIndexes()  # 선택된 파일들의 인덱스를 받아옴
    ...
    def createTopMenu(self):
        ...
        # 상단 메뉴바의 Edit를 클릭하면 Select All매뉴가 보이도록
        selectAllAction.setStatusTip('Select All')
        # selectAllAction이 trigger될 경우 self.selectAll실행
        selectAllAction.triggered.connect(self.selectAll)

        unselectAllAction = QAction('&Unselect All', self)
        # 상단 메뉴바의 Edit를 클릭하면 Unselect All매뉴가 보이도록
        unselectAllAction.setStatusTip('Unselect All')
        # unselectAllAction이 trigger될 경우 self.unselectAll실행
        ...
        # 상단 메뉴바의 View를 클릭하면 Icons매뉴가 보이도록
        iconsViewAction.setStatusTip('Change view as Icon only')
        ...
        # 상단 메뉴바의 View를 클릭하면 List매뉴가 보이도록
        listViewAction.setStatusTip('Change view as File name only')
        ...
        # 상단 메뉴바의 View를 클릭하면 Details매뉴가 보이도록
        detailViewAction.setStatusTip('Change view as Details (default)')
        ...
        # Git
        # 상단 메뉴바의 Git을 클릭하면 Repository Create매뉴가 보이도록
        gitInitAction = QAction('&Git Init', self)
        # 하단 메뉴바에 해당 메뉴에 대한 설명 표시
        gitInitAction.setStatusTip('Execute <git init> command')
        # gitInitAction이 triggger될경우  self.GitInit
        gitInitAction.triggered.connect(self.GitInit)
        ...
        gitRestoreAction.setStatusTip(
            'Execute <git restore [selected]> or <git restore --staged [selected]> command')
        # gitRestoreAction.triggered.connect(self.GitRestore)

        gitRmAction = QAction('&Rm', self)
        gitRmAction.setStatusTip(
            'Execute <git rm --cached [selected]> command')
        # gitInitAction.triggered.connect(self.GitRm)

        gitCommitAction = QAction('&Commit', self)
        gitCommitAction.setStatusTip(
            'Confirm about staged files and Execute <git commit -m [message]> command')
        # gitCommitAction.triggered.connect(self.GitCommit)

        # gitMenu(menuBar.addMenu("&Git"))에 Qaction추가
        gitMenu.addAction(gitInitAction)
        ...
        # aboutAction이 triggger될경우  self.about실행//안내메시지 창 띄움
        aboutAction.triggered.connect(self.about)

        # helpMenu(menuBar.addMenu("&Help"))에 Qaction추가
        helpMenu.addAction(aboutAction)


if __name__ == '__main__':  # 프로그램 실행시 실행되는 부분
    app = QApplication(sys.argv)  # QApplication생성
    # App생성 #Path.home()은 사용자의 홈디렉토리를 의미 - 홈디렉토리는 윈도우 11 기준 사용자 폴더 #ex는 App객체
    ex = App(Path.home())
    # 프로그램 종료시까지 실행 #app.exec_()는 이벤트 루프를 실행시키는 함수 #sys.exit()는 프로그램 종료
    sys.exit(app.exec_())
